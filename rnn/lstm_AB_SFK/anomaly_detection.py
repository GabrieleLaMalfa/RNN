...
def lstm_exp(filename, num_units, window, batch_size=3, l_rate=.01,
    ...
    # declare variable shapes: weights and bias
    ...
    input_ = tf.unstack(x, batch_size, 1)

    # define layers
    lstm_layer = rnn.BasicLSTMCell(num_units, forget_bias=1)
    outputs, _ = rnn.static_rnn(lstm_layer, input_, dtype="float32")

    # prediction
    y_hat = tf.matmul(outputs[-1], weights) + bias
